...
namespace Td.Dps
{
    ...
    public class Wired2 : IDps
    {
        ...
        /// </summary>
        private const string T_TXD1 = "COM";

        /// <summary>
        /// 送信コマンド：従来式：LEDのみ
        /// </summary>
        private const string T_TXD2 = "LMP";

        /// <summary>
        /// 送信コマンド：従来式：セグメントのみ
        /// </summary>
        private const string T_TXD3 = "COM";
        ...
        /// 受信コマンド：従来式：ボタン押下
        /// </summary>
        private const string R_BAR = "BAR";

        /// <summary>
        /// 受信コマンド：エラー
        /// </summary>
        private const string R_ERR = "ERR";
        ...
        /// バーコード受信通知のイベントハンドラ
        /// </summary>
        public event Event.BarCoodeEventHandler BarCodeRcv;
        ...
        private void Write(CommandList command)
        {
            ...
            {
                // パケットIDの取得
                string pid = packet_id.ToString("X2");

                // 送信文字列を取得
                string str = (command.Command + pid);
        ...
        private void ReceiveThreadStart()
        {
            ...
            this.receiveThread.Priority = ThreadPriority.BelowNormal;        // 受信スレッドの優先順位を上げる
        ...
        private void DataAnalyze()
        {
            ...
            try
            {
                ...
                switch (cmd)
                {
                    ...
                    case R_ERR:
                        this.DataAnalyzeError(str);
                        break;

                    case R_BAR:
                        this.DataAnalyzeBarCode(str);
                        break;
        ...
        private void DataAnalyzeButtonPush(string data)
        {
            ...
            {
                // データ分解
                info.Group = Convert.ToInt32(data.Substring(3, 2));
                info.Address = Convert.ToInt32(data.Substring(5, 3));
                info.Button = Convert.ToInt32(data.Substring(8, 1));
                info.Data = data;
        ...
        /// </summary>
        /// <param name="data">受信データ</param>
        private void DataAnalyzeError(string data)
        {
            Def.EventInfo info = new Def.EventInfo();
        ...
        /// </summary>
        /// <param name="data">受信データ</param>
        private void DataAnalyzeBarCode(string data)
        {
            Def.EventInfo info = new Def.EventInfo();
            ...
            try
            {
                ...
                info.Address = Convert.ToInt32(data.Substring(5, 3));
                info.Button = len;
                info.Data = data.Substring(9, len);

                // イベント発生
                this.BarCoodeEvent(info);
            }
            catch
            {
                throw;
        ...
        /// </summary>
        /// <param name="info">イベント情報</param>
        private void BarCoodeEvent(Def.EventInfo info)
        {
            if (this.BarCodeRcv != null)
            {
                info.BarCodeRcv = this.BarCodeRcv;
                this.worker.ResponseEvent(info);
            }
        }
