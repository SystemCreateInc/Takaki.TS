        ...
        public static List<DistInfo>? LoadInfs(DistGroup distgroup)
        {
            using (var con = DbFactory.CreateConnection())
            {
                ...
                var sql = @"select CD_DIST_GROUP,DT_DELIVERY,TB_DIST.CD_HIMBAN,ST_BOXTYPE,NU_BOXUNIT,max(NM_HIN_SEISHIKIMEI) NM_HIN_SEISHIKIMEI, CD_GTIN13,CD_GTIN14,CD_SHUKKA_BATCH,CD_JUCHU_BIN"
                        ...
                        + ",sum(case when tdunitzonecode = 1 then NU_DOPS-NU_DRPS else 0 end) leftpscnt"
                        + ",sum(case when tdunitzonecode = 2 then NU_DOPS-NU_DRPS else 0 end) rightpscnt"
                        + " from TB_DIST"
                ...
                var r =  con.Query(sql, new
                {
                    ...
                    scancode = scancode,
                     ...
                     .Select(q => new DistItemSeq
                     {
                         ...
                         LStatus = q.lstatus,
                         Ops = q.ops ?? 0,
                         Dops = bCheck || bExtraction ? q.drps : q.dops ?? 0,
                         Drps = bCheck || bExtraction ? 0 : q.drps ?? 0,
                         Ddps = bCheck || bExtraction ? q.drps : q.dops - q.drps,
                         Lrps = q.lrps ?? 0,
                         Order_shop_cnt = q.order_shop_cnt ?? 0,
